1.将两个字符利用字符串对象的方法变成一个字符,显示在页面id为h1的元素中
答:
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Document</title>
</head>
<body>
    <h1 id="h1"></h1>
    <script>
        var arr ='123';
        var brr ='456';
        console.log(arr.concat(brr));
        crr =arr.concat(brr);
        var drr =document.getElementById('h1');
        drr.innerHTML=crr;
    </script>
</body>
</html>
2.一个富豪想存87万,给理财顾问写了87w,请自动生成存储870000的方法,显示在页面id为h2的元素中
答:
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Document</title>
</head>
<body>
    <h2 id="h2"></h2>
    <script>
        alert('87w');
        var arr =Number(870000);
        var brr =document.getElementById('h2');
        brr.innerHTML='您现在的余额为'+arr+'元';
   
    </script>
</body>
</html>
3.一个数字79387.348的工程款,保留两位小数存入,显示在页面id为h3的元素中
答:
<!DOCTYPE html>
<html>
<head>
     <meta charset="UTF-8">
     <title>Document</title>
     <style></style>
</head>
<body>
    <h3 id="h3"></h3>
    <script>
    var money = 79387.348
    var saveMoust = money.toFixed(2);   
    h3.innerHTML = saveMoust;
    </script>
</body>
</html>
4.一张图片是一个相对路径img/head/icon/1.jpg,我只需要拿到它的文件夹目录后显示在页面id为h4的元素中
答:

5.用户输入验证码,无论大小写输入都会正确的方法,显示在页面id为h5的元素中,显示在页面id为h5的元素中
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Document</title>
</head>
<body>
    <h5 id="h4"></h5>
    <script>
        
        var a= prompt("请输入验证码");
        if(a.toLocaleLowerCase == 'abc'||a.toLocaleUpperCase == 'ABC'){
            var arr =document.getElementById('h4');
            arr.innerHTML =a;
        }else{
            var arr =document.getElementById('h4');
            arr.innerHTML =a;
        }
 
    </script>
    
</body>
</html>